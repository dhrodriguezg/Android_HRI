...
public class TwistTopic extends AbstractTopic {
    ...
    protected void publish(){
        ...
        twist.getLinear().setX(publisher_linear[1]);
        twist.getLinear().setY(publisher_linear[0]);
        twist.getLinear().setZ(publisher_linear[2]);
        twist.getAngular().setX(publisher_angular[1]);
        twist.getAngular().setY(publisher_angular[0]);
    ...
    protected void setupSubscriber(ConnectedNode connectedNode) {
        ...
        subscriber.addMessageListener(new MessageListener<Twist>() {
            ...
            public void onNewMessage(Twist twist) {
                subcriber_angular[1] = (float) twist.getAngular().getX();
                subcriber_angular[0] = (float) twist.getAngular().getY();
                subcriber_angular[2] = (float) twist.getAngular().getZ();
                subcriber_linear[1] = (float) twist.getLinear().getX();
                subcriber_linear[0] = (float) twist.getLinear().getY();
