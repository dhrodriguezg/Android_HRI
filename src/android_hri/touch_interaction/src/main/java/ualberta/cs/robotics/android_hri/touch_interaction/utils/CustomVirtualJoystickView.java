/*
 * This is a customized version of org.ros.android.view.VirtualJoystickView
 * The changes include:
 * -Fixed a bug when 2 or more VirtualJoystickView are used on the screen at the same time and it keeps publishing data when released (that's in onTouchEvent method).
 * -Getter methods for Joystick axes values, that way publishing the Topic is optional when you need to read the values inside your app.
 * -Removed Odometry reading.
 * -Previows velocity now Hidden.
 * -Previows intensity text now Hidden.
 */
 ...
 */
public class CustomVirtualJoystickView extends RelativeLayout implements AnimationListener, NodeMain {

    private static final String TAG = "CustomVirtualJoystick";
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_MOVE: {
                //Log.i(TAG, "ACTION_MOVE:" + topicName);
    ...
    private void onContactMove(float x, float y) {
        ...
        if (holonomic) {
            publishVelocity(normalizedMagnitude * Math.cos(contactTheta * Math.PI / 180.0), normalizedMagnitude * Math.sin(contactTheta * Math.PI / 180.0), 0);
        } else {
            publishVelocity(normalizedMagnitude * Math.cos(contactTheta * Math.PI / 180.0), 0, normalizedMagnitude * Math.sin(contactTheta * Math.PI / 180.0));
        }
    ...
    private void publishVelocity(double linearVelocityX, double linearVelocityY, double angularVelocityZ) {
        axis_x=linearVelocityX;
        axis_y=linearVelocityY;
        axis_z=angularVelocityZ;
    }
    ...
    private void updateMagnitudeText() {
        ...
        if (!turnInPlaceMode) {
            ...
            magnitudeText.setTranslationX((float) (parentSize / 4 * Math.cos((90 + contactTheta) * Math.PI / 180.0)));
            magnitudeText.setTranslationY((float) (parentSize / 4 * Math.sin((90 + contactTheta) * Math.PI / 180.0)));
        }
    }
    ...
    private boolean inLastContactRange(float x, float y) {
        if (Math.sqrt((x - contactUpLocation.x - joystickRadius) * (x - contactUpLocation.x - joystickRadius) + (y - contactUpLocation.y - joystickRadius) * (y - contactUpLocation.y - joystickRadius)) < THUMB_DIVET_RADIUS) {
            return true;
        }
        return false;
    }
    ...
    public void onStart(ConnectedNode connectedNode) {
        ...
        publisherTimer.schedule(new TimerTask() {
            ...
            public void run() {
                if (publishVelocity) {
                    currentVelocityCommand.getLinear().setX(axis_x);
                    currentVelocityCommand.getLinear().setY(-axis_y);
                    currentVelocityCommand.getLinear().setZ(0);
                    currentVelocityCommand.getAngular().setX(0);
                    currentVelocityCommand.getAngular().setY(0);
                    currentVelocityCommand.getAngular().setZ(-axis_z);
                    publisher.publish(currentVelocityCommand);
                }
            }
        }, 0, 80);
